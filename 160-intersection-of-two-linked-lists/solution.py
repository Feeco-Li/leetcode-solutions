...
# 6,1,8,4,5]. There are 2 nodes before the intersected node in A; There are 3 node
# s before the intersected node in B.
# - Note that the intersected node's value is not 1 because the nodes with value 1
#  in A and B (2^{nd} node in A and 3^{rd} node in B) are different node reference
# s. In other words, they point to two different locations in memory, while the no
# des with value 8 in A and B (3^{rd} node in A and 4^{th} node in B) point to the
#  same location in memory.
#
# **Example 2:**
#
# **Input:** intersectVal = 2, listA = [1,9,1,2,4], listB = [3,2,4], skipA = 3, sk
